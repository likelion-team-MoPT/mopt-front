...
const Analytics: React.FC = () => {
  ...
  const reportData: Record<PeriodType, ReportData> = {
    week: {
      ...
      recommendations: [
        { id: 1, title: '주말 저녁 방문 유도 프로모션 제안', subtitle: '주말 저녁 할인', type: '주말 저녁 할인' },
        { id: 2, title: '신규 상권 대상 타겟 광고 전략', subtitle: '신규 상권 타겟', type: '신규 상권 타겟' },
        { id: 3, title: 'SNS 이벤트 참여 유도 캠페인', subtitle: 'SNS 이벤트', type: 'SNS 이벤트' },
        { id: 4, title: '고객 맞춤 이벤트 캠페인 제안', subtitle: '이벤트 캠페인', type: '이벤트 캠페인' },
        { id: 5, title: '점심 타임 직장인 타겟 쿠폰 발송', subtitle: '직접인 타겟', type: '직접인 타겟' }
      ],
      channelComparison: {
        totalRevenue: 720000,
        channels: [
    ...
    thisMonth: {
      ...
      recommendations: [
        { id: 1, title: '재방문 고객 유도 캠페인', subtitle: '재방문 유도', type: '재방문 유도' },
        { id: 2, title: 'SNS 바이럴 확산 캠페인', subtitle: 'SNS 바이럴', type: 'SNS 바이럴' }
      ],
      channelComparison: {
    ...
    <Layout showBottomTab={false}>
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-4 py-6 space-y-6">
          {/* Overall Performance */}
          <div>
            <h2 className="text-lg font-semibold text-gray-900 mb-4">전체 성과</h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-yellow-400 rounded-lg p-4">
                <div className="text-white">
                  <p className="text-xs opacity-90">ROAS</p>
                  <p className="text-2xl font-bold">{currentData.roas}%</p>
                </div>
              </div>
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <p className="text-xs text-gray-500 mb-1">총 클릭 수</p>
                <p className="text-xl font-bold text-gray-900">{formatNumber(currentData.totalClicks)}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <p className="text-xs text-gray-500 mb-1">총 지출</p>
                <p className="text-lg font-bold text-gray-900">{formatCurrency(currentData.totalRevenue)}</p>
              </div>
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <p className="text-xs text-gray-500 mb-1">총 광고 수익</p>
                <p className="text-lg font-bold text-gray-900">{formatCurrency(currentData.totalAdSpend)}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-4">상위 5개 성과 캠페인(ROAS 기준)</h3>
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-20 bg-gray-100 rounded-full h-2 relative overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min(campaign.percentage / 6, 100)}%` }}
                      ></div>
                    </div>
                    <span className="text-sm font-medium text-gray-900 w-12 text-right">{campaign.percentage}%</span>
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={rec.id} className="bg-white rounded-lg p-4 shadow-sm">
                  <div className="flex items-center">
                    <div className="w-8 h-8 bg-yellow-400 rounded-lg flex items-center justify-center mr-3">
                      <span className="text-white text-sm font-bold">{rec.id}</span>
                    </div>
          ...
          {/* Channel Comparison */}
          <div className="pb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">채널별 성과 비교</h3>
            <p className="text-sm text-gray-600 mb-4">채널별 수익</p>
            <p className="text-2xl font-bold text-gray-900 mb-6">총 {formatCurrency(currentData.channelComparison.totalRevenue)}</p>
            
            <div className="bg-yellow-50 rounded-lg p-6">
              <div className="flex justify-center space-x-8 mb-6">
                {currentData.channelComparison.channels.map((channel, index) => (
                  <div key={index} className="text-center">
                    <div className={`w-6 h-24 ${channel.color} rounded-full mx-auto mb-2 relative`}>
                      <div 
